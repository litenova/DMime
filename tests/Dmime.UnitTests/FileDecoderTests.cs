...
public class ImageDecoderTests
{
    ...
    {
        var stream = File.OpenRead(Path.Combine("Files", "bmp-sample-512w-512h.bmp"));
    ...
    {
        var stream = File.OpenRead(Path.Combine("Files", "jpg-sample-313w-234h.jpg"));
    ...
    {
        var stream = File.OpenRead(Path.Combine("Files", "png-sample-300w-300h.png"));
    ...
    {
        var stream = File.OpenRead(Path.Combine("Files", "webp-sample-550w-404h.webp"));
    ...
    {
        var stream = File.OpenRead(Path.Combine("Files", "mwf-sample.wmf"));
    ...
    {
        var stream = File.OpenRead(Path.Combine("Files", "tiff-sample.tiff"));

        var result = await stream.DetectMimeTypeAsync();

        result.MimeType.Should().Be("image/tiff");
        result.FileExtensions.Should().Contain(".tiff");
    }

    [Fact]
    public async Task mp4_should_be_detected_correctly()
    {
        var stream = File.OpenRead(Path.Combine("Files", "mp4-sample.mp4"));
    ...
    {
        var stream = File.OpenRead(Path.Combine("Files", "pdf-sample.pdf"));
    ...
    {
        var stream = File.OpenRead(Path.Combine("Files", "mp3-sample.mp3"));
    ...
    {
        var stream = File.OpenRead(Path.Combine("Files", "ico-sample.ico"));
    ...
    {
        var stream = File.OpenRead(Path.Combine("Files", "emf-sample.emf"));
